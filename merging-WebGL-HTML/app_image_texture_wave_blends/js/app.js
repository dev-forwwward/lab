  ...
  addObjects() {
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      void main()	{
        ...
        // vNoise is returning vals between -1 and 1

        vec3 finalColor = mix(color1, color2, 0.7*(vNoise+1.));

        vec2 newUV = vUv;

        // added distortion to image in y values
	      newUV = vec2(newUV.x, newUV.y + 0.01*sin(newUV.x*10. + time));

	      vec4 imgView = texture2D(planeTexture,newUV);

        // RESULT
        // we can add, multiply (...) noise to the image texture
        gl_FragColor = imgView + vec4(vNoise);
        gl_FragColor = imgView * vec4(vNoise);
      }
`,
