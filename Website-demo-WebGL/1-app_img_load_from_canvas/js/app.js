...
export default class Sketch {
  ...
  addObjects() {
    // this will produce a 100x100px rectangle with 100 vertex in total (10 for each row and column) 
    ...
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        time: { value: 0 },
        planeTexture: { value: new THREE.TextureLoader().load('../img/ocean.jpg') },
      },
...
float cnoise(vec3 P){
  ...
  return 2.2 * n_xyz;
      ...
      void main() {
        ...
        float noise = cnoise(vec3(position.x*4.,position.y*4. + time/30.,0.));

        // RESULT
        // take each vertex in the wireframe (coordenates stored in uv)
        // measure distance from uv relative to the center ( [0.0] represented as vec2(0.5) )
        // note: plane takes up space between 0 and 1, therefore its center is at 0.5

        float dist = distance(uv, vec2(0.5));

        //newposition.z += 0.01*sin(dist*20.-time);
